...
export default function Home() {
  ...
  const r = useMemo(() => {
    ...
    return { map, pp, shockIndex, bsa, vtiChange, dco2, perfusion, responsive, tolerance, phenotype, actions };
  }, [f]);

  const input = (key: string, label: string, unit = "") => (
    <label><span>{label}{unit && <small> ({unit})</small>}</span><input type="number" value={f[key]} onChange={e => set(key, e.target.value)} /></label>
  );
  ...
  return (
    <main>
      ...
      <section><h2>1. Patient and pressure</h2><div className="grid">{input("height", "Height", "cm")}{input("weight", "Weight", "kg")}{input("sbp", "Systolic BP", "mmHg")}{input("dbp", "Diastolic BP", "mmHg")}{input("hr", "Heart rate", "/min")}</div>
        <div className="metrics"><Metric name="BSA" value={fmt(r.bsa, 2)} /><Metric name="MAP" value={fmt(r.map)} /><Metric name="Pulse pressure" value={fmt(r.pp)} /><Metric name="Shock index" value={fmt(r.shockIndex, 2)} /></div>
      </section>

      <section><h2>2. Global flow and intervention response</h2><div className="grid">{input("vtiPre", "Pre-intervention LVOT VTI", "cm")}{select("intervention", "Intervention", ["None", "100 mL mini-fluid challenge over 1 min", "250 mL fluid challenge over 2–3 min", "Other intervention"])}{input("vtiPost", "Post-intervention LVOT VTI", "cm")}{input("scvo2", "ScvO₂", "%")}{input("pco2v", "Central venous PCO₂", "mmHg")}{input("pco2a", "Arterial PCO₂", "mmHg")}</div>
